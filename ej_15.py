...
def getCantPokemonsDeUnEntrenador(nombreEntrenador, objLista):
...
def mostrarCantPokemonsDeUnEntrenador(nombreEntrenador, objLista):
    cantidadPokemons = getCantPokemonsDeUnEntrenador(nombreEntrenador, objLista)
...
def mostrarEntrenadoresConMasTresTorneos(objLista):
...
def getPokemonMasNivelDeEntrenador(entrenador):
    pokemon = None
    lvlPokemon = 0
...
def mostrarDatosEntrenadorySusPokemones(entrenador):
...
def EntrenadoresConPokemonesDeXTipoySubtipoOtroTipoySubtipo(objLista, tipo1, subtipo1, tipo2, subtipo2):
    entrenador = None
    pokemon = None
...
def mostrarPromedioNivelDeLosPokemones(objLista):
    sumaLvls = 0
...
def mostrarCantDeEntrenadoresQueTienenDetPokemon(objLista, nombrePokemon):
...
mostrarCantPokemonsDeUnEntrenador('Ash', listaEntrenadores)

print('\nLISTAR ENTRENADORES CON MAS DE TRES TORNEOS GANADOS')
mostrarEntrenadoresConMasTresTorneos(listaEntrenadores)
...
pokemon = getPokemonMasNivelDeEntrenador(listaEntrenadores.obtener_elemento(indiceEntrenador))

print(pokemon)

print('\nMOSTRAR LA INFO DE UN ENTRENADOR Y EL DE SUS POKEMONES')
mostrarDatosEntrenadorySusPokemones(listaEntrenadores.obtener_elemento(3))
...
EntrenadoresConPokemonesDeXTipoySubtipoOtroTipoySubtipo(listaEntrenadores, 'fuego', 'planta', 'agua', 'volador')

print('\nPROMEDIO DE NIVELES DE LOS POKEMONES DE LOS ENTRENADORES')
mostrarPromedioNivelDeLosPokemones(listaEntrenadores)

print('\nDETERMINAR CUANTOS ENTRENADORES TIENEN EL MISMO POKEMON')
mostrarCantDeEntrenadoresQueTienenDetPokemon(listaEntrenadores, 'Raichu')
